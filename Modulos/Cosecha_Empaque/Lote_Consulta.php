	...
	<div class="table-responsive">
		<div class="table-wrapper">
			<div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Ingreso de Lotes</b></h2>
                    </div>
                </div>
				<div class="row">
                    <div class="col">
                        <input type="number" class="form-control" placeholder="Lote" id="filtroUsuario">
                    </div>
                    <div class="col">
                        <input type="number" class="form-control" placeholder="Superficie" id="filtroEstado">
                    </div>
                    <div class="col">
		...
		<div class="modal-content">
			<form>
				<div class="modal-header">
					<h4 class="modal-title">Ingreso Lote</h4>
				...
				<div class="modal-body">
                   <!--  <div class="form-group">
					...
					<div class="form-group">
						<label>Lote</label>
						<input id="txt_lote" type="Number" class="form-control editable" required>
						<p id="txt_lote1" class="error-message"></p>
					</div>
                    <div class="form-group">
						<label>Superficie</label>
						<input id="txt_superficie" type="Number" class="form-control editable" required>
						<p id="txt_superficie1" class="error-message"></p>
                        <input type="hidden" name="txt_id" id="txt_id" value="0"/>
                        <input type="hidden" name="txt_ideli" id="txt_id" value="0"/>
					</div>
...
<script src="../js/jquery-3.5.1.min.js"></script>
    <script>
        ...
        document.getElementById('txt_lote').addEventListener('input', function() {
            validarNumerosPositivos('txt_lote', 'txt_lote1');
        });
        document.getElementById('txt_superficie').addEventListener('input', function() {
            validarNumerosPositivos('txt_superficie', 'txt_superficie1');
        });
        $(document).ready(function() {
            ...
            function cargarUsuarios() {
                ...
                $.ajax({
                    url: "Cosecha_Empaque/Lote_Controlador.php",
                    method: "POST",
			...
			$(document).on("click", ".edit", function() {
                ...
                var lote = $userRow.find("td:eq(1)").text(); // Codigo Proveedor
				var superficie = $userRow.find("td:eq(2)").text(); // Cantidad
               
               
				// Llena los campos del modal con los valores obtenidos
				$("#txt_id").val(id);
				$("#txt_lote").val(lote);
                $("#txt_superficie").val(superficie);
			...
			$(document).on("click", ".delete", function() {
                ...
                Swal.fire({
                    ...
                    text: 'Se Procederá a realizar esta eliminacion de información',
                    icon: 'warning',
						...
						$.ajax({ 
							...
							success:function(datos){
								...
								if(datos === '1'){
									...
									mensaje(titulo_error, 'No se Realizo el proceso', 'error');
								}
							}
						});
			...
			$("#btn_ingreso").click(function(){
				...
				if(!validarNumeroDecimal(lote) || parseFloat(lote) < 0 ){
					mensaje(titulo_error, 'El Lote debe ser positivo', 'error');
					return;
				}
                if(!validarNumeroDecimal(superficie) || parseFloat(superficie) < 0 ){
					mensaje(titulo_error, 'L Superficie debe ser positivo', 'error');
					return;
				}
