    ...
    <script>
        $(document).ready(function() {
			...
			$("#txt_descripcion").on("input", function() {
				validarCampoLetras($(this), 'Nombres');
			});

			$("#txt_usuario").on("input", function() {
				validarCampoLetras($(this), 'Usuario');
			});
			// Función para validar que solo se ingresen letras en un campo
			function validarCampoLetras(elemento, nombreCampo) {
				var valor = elemento.val();
				var mensaje = '';

				if (!/^[A-Za-zñÑáéíóúÁÉÍÓÚ\s]+$/.test(valor)) {
				mensaje = 'El campo ' + nombreCampo + ' solo permite letras y espacios, corrija.';
				}
			...
			function validarEmail(elemento) {
				...
				mensaje = 'Ingrese un correo electrónico válido, corrija.';
				}
			...
			function mostrarError(elemento, mensaje) {
				elemento.next(".error-message").text(mensaje);
			}
			...
			$("#btn_ingreso").click(function(){
				...
				if (!/^[A-Za-zñÑáéíóúÁÉÍÓÚ\s]+$/.test(usuario)) {
					mensaje('Error', 'El campo Usuario solo permite letras y espacios', 'error');
					return;
				}
				// Validar que el email sea válido
				if (!isValidEmail(email)) {
					mensaje('Error', 'Ingrese un correo electrónico válido, corrija.', 'error');
					return;
				}
				if (id==0)
				{
